/**
 * @file KeyTest.cpp
 * @brief Power, button, and encoder input handling
 */
...
bool FactoryTest::_check_test_mode()
{
    ...
    while (!_btn_pwr.read())
    {
        ...
        if (_is_test_mode)
        {
            ...
            _canvas->printf("[DinMeter] Test Firmware\n%s (%s)\n\nRelease button to enter test mode", FW_VERSION, __DATE__);
            _canvas_update();
...
void FactoryTest::_encoder_test()
{
    ...
    _canvas->printf(" [Encoder Test]\n");
    _canvas->setTextColor(TFT_WHITE);
    _canvas->printf(" Turn to target\n then press button\n");
